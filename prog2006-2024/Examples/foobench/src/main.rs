// This code in inspired by the code from
// https://github.com/ThomasMeissnerDS/cpu_benchmark
//
use std::env;
use std::f64;
...
fn main() {
    ...
    println!("Total runtime: {:.2?}", elapsed.as_millis());
    println!("GIGA Calculations per second: {:.2?} seconds.", calc_per_sec / 1000000000.0);
    println!("Ratio for Mariusz's machine: {:.2?}", elapsed.as_millis() as f64 / 6360.0);
}